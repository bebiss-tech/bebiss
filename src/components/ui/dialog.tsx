...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      "fixed inset-0 z-20 bg-white/80 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 dark:bg-slate-950/80",
      className
    )}
    {...props}
  />
...
const DialogContent = React.forwardRef<
  ...
  DialogContentProps
>(({ className, children, welcomeFlow, ...props }, ref) => (
  <DialogPortal>
    {!welcomeFlow && <DialogOverlay />}
    <DialogPrimitive.Content
      onPointerDownOutside={(e) => {
        if (welcomeFlow) e.preventDefault();
      }}
      onEscapeKeyDown={(e) => {
        if (welcomeFlow) e.preventDefault();
      }}
